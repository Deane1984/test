...
void hal_entry(void)
{
    ...
    //if(!((direct_command.data == 0xEF18) || (direct_command.data == 0x18EF)))//For CPKDIS W25Q128JW
    {
        __BKPT(0);
    }
    ...
    memset(&direct_command, 0, sizeof(direct_command));
    direct_command.command        = 0xEB;
    direct_command.command_length = 0x1;
    direct_command.address        = 0x04000300U;
    direct_command.address_length = 0x1;
    direct_command.dummy_cycles   = 0x1;
    direct_command.data_length    = 0x1;
    err = R_OSPI_B_DirectTransfer (g_ospi0.p_ctrl, &direct_command, SPI_FLASH_DIRECT_TRANSFER_DIR_READ);
    fsp_assert (err);
